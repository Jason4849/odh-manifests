...
isPodReady(){
  ...
  ready_pod=$(oc get pod -n ${prj_name} |grep $pod_name  --no-headers|awk '{print $2}' |cut -d/ -f1)
  desired_pod=$(oc get pod -n ${prj_name} | grep $pod_name  --no-headers|awk '{print $2}' |cut -d/ -f2)
